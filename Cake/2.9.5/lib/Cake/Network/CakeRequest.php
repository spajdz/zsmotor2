 ...
 */
	protected function _base() {
                /**
                 * Detecta ejecucion via CLI (CAPTURA TRANSBANK)-- fix bside 2015-09-09 10:58am
                 */
                $fix_host = env('HTTP_HOST');
                if ( empty($fix_host) )
                {
                        $fix_parse = parse_url(env('REQUEST_URI'));
                        if ( ! empty($fix_parse['scheme']) )
                        {
                                define('FULL_BASE_URL', sprintf('%s://%s', $fix_parse['scheme'], $fix_parse['host']));
                                Configure::write('App.fullBaseUrl', FULL_BASE_URL);

                                $_SERVER['REQUEST_URI'] = $fix_parse['path'];
                                $_SERVER['HTTP_HOST'] = $fix_parse['host'];
                        }
                        unset($fix_host, $fix_parse);
                }
                /**
                 * Fin fix
                 */
