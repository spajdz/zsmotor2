...
App::uses('View', 'View');
class CompraDespachoListener implements CakeEventListener
{
	public $View				= null;
	...
	{
		return array(
			'Model.CompraDespacho.preparacionBodega'	=> 'enviarEmail'
		);
	}
	...
	public function enviarEmailUsuario(CakeEvent $evento)
	{
		...
		$html						= $this->View->render('preparacion_bodega_oc_usuario', 'default');
		...
		$this->Email->save(array(
			'asunto'					=> sprintf('Notificación de preparación bodega - OC %d', $compra['Compra']['id']),
