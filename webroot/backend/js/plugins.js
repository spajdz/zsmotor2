$(function() {
    ...
    var formElements = function(){
        ...
        var feDatepicker = function(){
			...
			/**
			 *	Calendarios utilizados en el buscador de OC
			 *	Estos calendarios definen el inicio y de cada uno, es decir,
			 *	dependiendo de la fecha seleccionada en _start_oc, desde esa fecha
			 *	inicia _end_oc y viceverza
			 */
			/*

            if($(".datepicker_start_oc").length > 0){
                $(".datepicker_start_oc").datepicker({
					format: 'yyyy-mm-dd'
				}).on('changeDate', function(data){
						$('.datepicker_end_oc').datepicker('setStartDate', data.date);
				});
            }

			if($(".datepicker_end_oc").length > 0){
                $(".datepicker_end_oc").datepicker({
					format: 'yyyy-mm-dd'
				}).on('changeDate', function(data){
						$('.datepicker_start_oc').datepicker('setEndDate', data.date);
				});
            }
            */
			 ...
			 */
			/*
			if($(".datepicker_start_reserva").length > 0){
                $(".datepicker_start_reserva").datepicker({
					format: 'yyyy-mm-dd'
				}).on('changeDate', function(data){
						$('.datepicker_end_reserva').datepicker('setStartDate', data.date);
				});
            }

			if($(".datepicker_end_reserva").length > 0){
                $(".datepicker_end_reserva").datepicker({
					format: 'yyyy-mm-dd'
				}).on('changeDate', function(data){
						$('.datepicker_start_reserva').datepicker('setEndDate', data.date);
				});
            }
            */
