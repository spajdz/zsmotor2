...
class ComprasController extends AppController
{
	...
	public function webpay_kcc()
	{
		...
		$webpay			= array(
			...
			'exito'				=> Router::url(array('controller' => 'compras', 'action' => 'exito'), true),
			'fracaso'			=> Router::url(array('controller' => 'compras', 'action' => 'fracaso'), true)
		);
	...
	public function fracaso_kcc()
	{
		...
		// prx($this->Session->read('Flujo.Carro.compra_id'));
	...
	public function cierre()
	{
		...
		{
			/**
			 * Valida que el POST venga de la maquina local
			 */
			$validado		= ($this->request->clientIp() == env('SERVER_ADDR'));

			/**
			 * Valida que exista la compra y exista en estado pendiente
			 */
			if ( $validado )
			{
				extract($this->request->data);
				$id					= $TBK_ORDEN_COMPRA;
				$reversado			= ($TBK_RESPUESTA == 0);
				$validado			= $this->Compra->pendiente($id);
			}

			/**
			 * Valida estado informado por Transbank
			 */
			if ( $validado )
			{
				if ( $TBK_RESPUESTA != 0 )
				{
					$aceptado = true;
					$this->Compra->cambiarEstado($id, 'RECHAZO_TRANSBANK', 'Transacción rechazada por Transbank', $aceptado);
					$validado = false;
				}
			}

			/**
			 * Valida los campos que deben venir en el POST desde Transbank
			 */
			if ( $validado )
			{
				foreach ( $this->Transbank->camposPost as $campo )
				{
					if ( $this->request->data($campo) === null )
					{
						$this->Compra->cambiarEstado($id, 'RECHAZO_COMERCIO', 'POST Transbank incompleto', $aceptado, $reversado);
						$validado = false;
						break;
					}
				}
			...
			if ( $validado )
			{
				...
				if ( ! $this->Compra->validates() || ! $this->Compra->save($data) )
				{
					$this->Compra->cambiarEstado($id, 'RECHAZO_COMERCIO', 'POST Transbank no pasa validación DB', $aceptado, $reversado);
					$validado = false;
				}
			}

			/**
			 * Valida monto de compra
			 */
			if ( $validado )
			{
				$compra			= $this->Compra->findById($id);
				if ( sprintf('%d00', $compra['Compra']['total']) != $TBK_MONTO )
				{
					$this->Compra->cambiarEstado($id, 'RECHAZO_COMERCIO', 'Monto informado por Transbank no concuerda', $aceptado, $reversado);
					$validado = false;
				}
			}

			/**
			 * Crea el archivo de validacion MAC
			 */
			if ( $validado )
			{
				$log = $this->Transbank->logMac($id, $this->request->data);
				if ( ! $log )
				{
					$this->Compra->cambiarEstado($id, 'RECHAZO_COMERCIO', 'Error al crear archivo de validación MAC', $aceptado, $reversado);
					$validado = false;
				}
			}

			/**
			 * Valida la firma digital MAC
			 */
			if ( $validado )
			{
				if ( ! $this->Transbank->validaMac($log) )
				{
					$this->Compra->cambiarEstado($id, 'RECHAZO_COMERCIO', 'MAC informada por Transbank no válida', $aceptado, $reversado);
					$validado = false;
				}
			}

			/**
			 * Actualiza el estado de la transacción a Aceptada
			 */
			if ( $validado )
			{
				$aceptado = true;
				$this->Compra->cambiarEstado($id, 'PAGADO', null, $aceptado, false);
			}
		}
		$this->set(compact('aceptado'));
	}

	public function exito_kcc()
	{
		/**
		 * Comprueba que venga informada la oc
		 */
		 ...
		 * y corresponda a la oc informada por webpay
		 */
		if ( ! ( $id = $this->Session->read('Flujo.Carro.compra_id') ) || $id != $TBK_ORDEN_COMPRA || ! $this->Compra->pagada($id) )
		{
			$this->redirect(array('action' => 'resumen'));
		}
